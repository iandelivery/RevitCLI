...
{
    /// <summary>
    /// Creates a cross fitting where two branch pipes meet a main pipe that
    ...
    /// connectors must meet at approximately the same point, and both branches
    /// must be perpendicular to the main within 1°.
    /// </summary>
    public class CreateCrossFittingHandler : DocumentCommandBase
    {
        public override string CommandName => "create_pipe_cross_fitting";
        public override string Description => "Creates a cross fitting at the meeting point of two main halves and two branch pipes";
        public override string Category => "Create";
        public override string[] Aliases => new[] { "pipe_cross_fitting" };
        public override bool SupportsDryRun => true;

        public override CommandParamSchema[] Parameters => new[]
        {
            new CommandParamSchema { Name = "main_element_id_1", Type = "int", Required = true, Description = "First half of the pre-split main pipe" },
            new CommandParamSchema { Name = "main_element_id_2", Type = "int", Required = true, Description = "Second half of the pre-split main pipe" },
            new CommandParamSchema { Name = "branch_element_id_1", Type = "int", Required = true, Description = "First branch pipe" },
            new CommandParamSchema { Name = "branch_element_id_2", Type = "int", Required = true, Description = "Second branch pipe" }
        };

        public override string[] Examples => new[]
        {
            "{ \"command\": \"create_pipe_cross_fitting\", \"parameters\": { \"main_element_id_1\": 12345, \"main_element_id_2\": 12346, \"branch_element_id_1\": 12347, \"branch_element_id_2\": 12348 } }"
        };

        protected override string Execute(UIApplication app, Document doc, Dictionary<string, object> parameters, QueuedCommand cmd)
        {
            var p = TryBind<PipeCrossParams>(cmd, out var error);
            ...
            var main1 = doc.GetElement(new ElementId(p.MainElementId1)) as Pipe;
            var main2 = doc.GetElement(new ElementId(p.MainElementId2)) as Pipe;
            var branch1 = doc.GetElement(new ElementId(p.BranchElementId1)) as Pipe;
            var branch2 = doc.GetElement(new ElementId(p.BranchElementId2)) as Pipe;
            if (main1 is null || main2 is null || branch1 is null || branch2 is null)
                return CommandResponse.Error(cmd.TaskId, "One or more element IDs do not refer to a pipe.").ToJson();
            ...
            if (main1Curve is null || main2Curve is null || branch1Curve is null || branch2Curve is null)
                return CommandResponse.Error(cmd.TaskId, "One or more pipes have no location curve.").ToJson();
            ...
            using var tx = new DryRunTransaction(doc, "CLI Create Pipe Cross Fitting", cmd.DryRun);
            try
            {
