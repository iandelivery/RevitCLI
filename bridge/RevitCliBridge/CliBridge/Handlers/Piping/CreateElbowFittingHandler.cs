...
{
    /// <summary>
    /// Creates an elbow fitting between two pipe connectors. The connectors
    /// must belong to different elements, share the same domain, and form an
    /// angle between 2° and 95°. The closest connector pair is auto-selected
    /// when explicit indices are not provided.
    /// </summary>
    public class CreateElbowFittingHandler : DocumentCommandBase
    {
        public override string CommandName => "create_pipe_elbow_fitting";
        public override string Description => "Creates an elbow fitting between two pipe connectors";
        public override string Category => "Create";
        public override string[] Aliases => new[] { "pipe_elbow_fitting" };
        ...
        {
            "{ \"command\": \"create_pipe_elbow_fitting\", \"parameters\": { \"element_id_1\": 12345, \"element_id_2\": 12346 } }"
        };

        protected override string Execute(UIApplication app, Document doc, Dictionary<string, object> parameters, QueuedCommand cmd)
        {
            var p = TryBind<PipeElbowParams>(cmd, out var error);
            ...
            using var tx = new DryRunTransaction(doc, "CLI Create Pipe Elbow Fitting", cmd.DryRun);
            try
            {
