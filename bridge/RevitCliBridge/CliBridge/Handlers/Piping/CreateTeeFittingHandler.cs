...
{
    /// <summary>
    /// Creates a tee fitting where a branch pipe meets a main pipe. The main
    /// is auto-split by Revit when the same main connector is passed twice to
    /// <c>NewTeeFitting</c> — no manual pre-splitting is required. The branch
    /// must be approximately perpendicular to the main (within 1°).
    /// </summary>
    public class CreateTeeFittingHandler : DocumentCommandBase
    {
        public override string CommandName => "create_pipe_tee_fitting";
        public override string Description => "Creates a tee fitting where a branch pipe meets a main pipe";
        public override string Category => "Create";
        public override string[] Aliases => new[] { "pipe_tee_fitting" };
        public override bool SupportsDryRun => true;

        public override CommandParamSchema[] Parameters => new[]
        {
            new CommandParamSchema { Name = "main_element_id", Type = "int", Required = true, Description = "Main pipe element ID (will be auto-split at the intersection)" },
            new CommandParamSchema { Name = "branch_element_id", Type = "int", Required = true, Description = "Branch pipe element ID" },
        ...
        {
            "{ \"command\": \"create_pipe_tee_fitting\", \"parameters\": { \"main_element_id\": 12345, \"branch_element_id\": 12346 } }"
        };

        protected override string Execute(UIApplication app, Document doc, Dictionary<string, object> parameters, QueuedCommand cmd)
        {
            var p = TryBind<PipeTeeParams>(cmd, out var error);
            ...
            if (mainCurve is null || branchCurve is null)
                return CommandResponse.Error(cmd.TaskId, "Pipe has no location curve.").ToJson();

            var intersection = MepCurveGeometry.ComputeIntersection(mainCurve, branchCurve);
            if (intersection is null)
                return CommandResponse.Error(cmd.TaskId, "Branch pipe does not intersect the main pipe.").ToJson();
            ...
            using var tx = new DryRunTransaction(doc, "CLI Create Pipe Tee Fitting", cmd.DryRun);
            try
            {
